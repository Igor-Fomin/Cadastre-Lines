...
namespace CadastreTools
{
    ...
    {
        public static Brush BackgroundBrush = new SolidColorBrush(Color.FromRgb(30, 30, 30));
        ...
        public static Border CreateCard() { return new Border() { Background = CardBrush, CornerRadius = new CornerRadius(4), Padding = new Thickness(10), Margin = new Thickness(0, 0, 0, 10), Effect = new DropShadowEffect() { BlurRadius = 8, ShadowDepth = 2, Opacity = 0.4 } }; }
        public static TextBox CreateInputBox() { return new TextBox() { Background = InputBackground, Foreground = Brushes.Cyan, FontFamily = new FontFamily("Consolas"), FontSize = 16, Height = 35, VerticalContentAlignment = VerticalAlignment.Center, BorderThickness = new Thickness(1), BorderBrush = Brushes.Gray, Padding = new Thickness(5), CaretBrush = Brushes.White }; }
    ...
    public class LKeyinCommand
    {
        ...
        {
            if (_palette == null || !_palette.IsLoaded) { _palette = new CadastreWpfWindow(); AcApp.ShowModelessWindow(_palette); }
    ...
    public static class CadMath
    {
        public static double ParseDmsToDegrees(double rawInput)
        {
            ...
            double minutes = double.Parse(decimalStr.Substring(0, 2)); double seconds = double.Parse(decimalStr.Substring(2, 2) + "." + decimalStr.Substring(4));
        ...
        public static double AddSubDms(double dms1, double dms2, bool add)
        {
            ...
            int d = (int)resDeg; double remainder = (resDeg - d) * 60.0;
            int m = (int)remainder; double s = (remainder - m) * 60.0;
            string sStr = s.ToString("00.00").Replace(".", "");
            return double.Parse($"{d}.{m:00}{sStr}");
        }
    ...
    public static class DwgDataManager
    {
        ...
        private static int ScanLayerForMaxPoint(Database db, Transaction tr)
        {
            ...
            try
            {
                ...
                if (!lt.Has(CadConstants.LAY_TXT_PTNUM)) return 0;
    ...
    public class CadastreWpfWindow : System.Windows.Window
    {
        ...
        {
            if (e.Source is TabControl && mainTabs.SelectedIndex != 1)
            {
        ...
        private void ToggleLayerVisibility(string layerName, bool isVisible)
        {
            ...
            using (Transaction tr = doc.TransactionManager.StartTransaction())
            {
                ...
                {
                    EnsureLayerExists(layerName, tr, doc.Database);
                }
                tr.Commit();
            }
        }
        ...
        private void PopulateComboBoxes()
        {
            try
            {
                ...
                foreach (var c in combos) c.ItemsSource = layers;
                foreach (var r in _textUiRows) r.CmbStyle.ItemsSource = styles;
            }
            catch { }
        }
        ...
        private void SetStartPoint(Point3d pt)
        {
            ...
            using (DocumentLock loc = AcApp.DocumentManager.MdiActiveDocument.LockDocument()) using (Transaction tr = AcApp.DocumentManager.MdiActiveDocument.TransactionManager.StartTransaction())
            {
                // FIX: Check if number exists before creating
                bool exists = DwgDataManager.IsPointNumberAtLocation(pt, tr, AcApp.DocumentManager.MdiActiveDocument.Database);

                if (!exists)
                {
                    int nextNum = DwgDataManager.GetNextPointNumber();
                    ...
                    Entity ptTxt = CreateText(nextNum.ToString(), CadConstants.LAY_TXT_PTNUM, pt, AttachmentPoint.BottomLeft, tr, AcApp.DocumentManager.MdiActiveDocument.Database, _config.TextPt);
                    AddToDb(ptTxt, btr, tr);
                    DwgDataManager.SetNextPointNumber(nextNum + 1);
                }
        ...
        private void ExecuteManualDraw()
        {
            ...
            using (DocumentLock loc = doc.LockDocument()) using (Transaction tr = doc.TransactionManager.StartTransaction())
            {
                try
                {
                    ...
                    // History Log - Estimate current point num
                    int currentNum = DwgDataManager.GetNextPointNumber() - 1;
                    lstHistory.Items.Insert(0, $"#{currentNum} | {txtAzimuth.Text}° | {txtDistance.Text}m");

                    UpdateRunningMisclosure(); CalculateArea(); PlayAudio(); PanToPoint(newPt); doc.Editor.UpdateScreen();
        ...
        private void UndoLastStep()
        {
            ...
            using (DocumentLock loc = doc.LockDocument()) using (Transaction tr = doc.TransactionManager.StartTransaction())
            {
                ...
                // Rollback DWG Number only if we erased a point number
                ...
                if (pointDeleted)
                {
                    int current = DwgDataManager.GetNextPointNumber();
                    if (current > 1) DwgDataManager.SetNextPointNumber(current - 1);
                }
                ...
                UpdateRunningMisclosure(); CalculateArea(); tr.Commit(); doc.Editor.UpdateScreen();
        ...
        private void EnsureLayerExists(string layerName)
        {
            ...
            using (doc.LockDocument()) using (Transaction tr = doc.TransactionManager.StartTransaction())
            {
                ...
                if (!lt.Has(layerName))
                {
                    ...
                    Autodesk.AutoCAD.Windows.ColorDialog cd = new Autodesk.AutoCAD.Windows.ColorDialog();
                    if (cd.ShowDialog() == System.Windows.Forms.DialogResult.OK) ltr.Color = cd.Color;
                    else ltr.Color = AcColor.FromColorIndex(Autodesk.AutoCAD.Colors.ColorMethod.ByAci, 7);
        ...
        private void EnsureLayerExists(string layerName, Transaction tr, Database db)
        {
            ...
            {
                lt.UpgradeOpen(); LayerTableRecord ltr = new LayerTableRecord(); ltr.Name = layerName;
                ltr.Color = AcColor.FromColorIndex(Autodesk.AutoCAD.Colors.ColorMethod.ByAci, 7);
                lt.Add(ltr); tr.AddNewlyCreatedDBObject(ltr, true);
            }
        }
        ...
        private void AddTextComment(string? preDefinedComment)
        {
            ...
            if (!string.IsNullOrWhiteSpace(finalComment))
            {
                ...
                using (DocumentLock loc = doc.LockDocument()) using (Transaction tr = doc.TransactionManager.StartTransaction())
                {
                    BlockTable bt = (BlockTable)tr.GetObject(doc.Database.BlockTableId, OpenMode.ForRead);
                    BlockTableRecord btr = (BlockTableRecord)tr.GetObject(bt[BlockTableRecord.ModelSpace], OpenMode.ForWrite);
                    Entity txt = CreateText(finalComment, CadConstants.LAY_TXT_SYMB, _lastCreatedVertex, AttachmentPoint.MiddleLeft, tr, doc.Database, _config.TextComm);
                ...
                using (DocumentLock loc = doc.LockDocument()) using (Transaction tr = doc.TransactionManager.StartTransaction())
                {
                    try
                    {
                        ...
                        if (!string.IsNullOrEmpty(radWin.Comment))
                        {
                            ...
                            Entity txt = CreateText(radWin.Comment, CadConstants.LAY_TXT_SYMB, endPt, AttachmentPoint.MiddleLeft, tr, doc.Database, _config.TextComm);
                            ObjectId txtId = AddToDb(txt, btr, tr);
                            if (_undoStack.Count > 0) _undoStack.Peek().Add(txtId);
                        }
                        tr.Commit(); doc.Editor.UpdateScreen();
